...
$(function(){
	...
	$("#min").focus(function(){
		...
		if($("#min").val()=='')
		{
			$("#minSpan").text("*必填项").css("color","red");
		}
	});
	/* 姓名验证 */
	$("#max").focus(function(){
		...
		if($("#max").val()=='')
		{
			$("#maxSpan").text("*必填项").css("color","red");
		}
	});
	/* 提交form */
	$("#savePermiss").bind("click", function()
	 {  
		if($("#min").val()=='')
		{
			$("#minSpan").text("*必填项").css("color","red");
				...
				<div class="span12">
					<div class="widget-box">
						<div class="widget-title">
							...
							<h5>权限列表</h5>
							<%-- <span class="label label-info"><a href="">修改</a></span> --%>
						</div>
									...
									<s:iterator value="list" id="per" status="stat">
										<s:if test='#per.userType=="0"'>
											<tr>
												...
												<td id="editLook<s:property value="#stat.index+1"/>"
													onclick="javascript:editTd('<s:property value="#stat.index+1"/>')"><s:if
														test='#per.isLook=="0"'>
														<i class="icon-remove"></i>
													</s:if> <s:elseif test='#per.isLook=="1"'>
														<i class="icon-ok"></i>
													</s:elseif></td>
												<td id="editExe"><s:if test='#per.isExe=="0"'>
														<i class="icon-remove"></i>
													</s:if> <s:elseif test='#per.isExe=="1"'>
														<i class="icon-ok"></i>
													</s:elseif></td>
												<td><span class="label label-info"><a
														href="permiss/permiss_getPermiss.action?code=<s:property value="#per.pk_id"/>">修改</a></span></td>
											</tr>
										</s:if>
									</s:iterator>
								</tbody>
									...
									<s:iterator value="list" id="per" status="stat">
										<s:if test='#per.userType=="1"'>
											<tr>
												...
												<td><s:if test='#per.isExe=="0"'>
														<i class="icon-remove"></i>
													</s:if> <s:elseif test='#per.isExe=="1"'>
														<i class="icon-ok"></i>
													</s:elseif></td>
												<td><span class="label label-info"><a
														href="permiss/permiss_deletePer.action?code=<s:property value="#per.pk_id"/>"
														onclick="javascript:return del_sure();">删除</a></span></td>
											</tr>
										</s:if>
									</s:iterator>
								</tbody>
					...
					<div class="widget-box">
						...
						<div class="widget-content nopadding">
							<form class="form-horizontal" method="post"
								action="permiss/permiss_savePermiss.action"
								name="number_validate" id="number_validate" novalidate>
								<div class="control-group">
									<label class="control-label">用户名</label>
									<div class="controls">
										<input type="text" name="pms.powUser" id="min" /><span
											id="minSpan"></span>
									</div>
								</div>
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" name="pms.powName" id="max" /><span
											id="maxSpan"></span>
									</div>
								</div>
								<div class="control-group">
								...
								<div class="form-actions">
									<input id="savePermiss" type="button" value="新增"
